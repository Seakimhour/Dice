...
(function(dice) {
    ...
    this.material_options = { specular: 0x172022, color: 0xf0f0f0, shininess: 40, shading: THREE.FlatShading };
    ...
    this.dice_box.prototype.roll = function(vectors, values, callback) {
        ...
        if (values != undefined && values.length) {
            ...
            // console.log(this.dices, values, res);
            this.prepare_dices_for_roll(vectors);
    ...
    this.dice_box.prototype.search_dice_by_mouse = function(ev) {
        ...
        let offsetX = ev.path[0].offsetParent.offsetLeft + ev.path[0].offsetLeft;
        let offsetY = ev.path[0].offsetParent.offsetTop + ev.path[0].offsetTop;

        let intersects = (new THREE.Raycaster(this.camera.position, (new THREE.Vector3((m.x - offsetX - this.cw) / this.aspect, 1 - (m.y - offsetY - this.ch) / this.aspect, this.w / 9)).sub(this.camera.position).normalize())).intersectObjects(this.dices);
        if (intersects.length) {
            let selectedDice = this.dices.filter(function (dice) { return dice.uuid == intersects[0].object.uuid});
            if (selectedDice == undefined) return selectedDice;
            
            let diceIndex = 0;
            while (this.dices[diceIndex].uuid != intersects[0].object.uuid) {
                diceIndex++;
            }
            return diceIndex;
        }
    ...
    this.dice_box.prototype.remove_dice_and_get_value = function(dice_index) {
        let selectedDice = this.dices[dice_index];

        this.dices = this.dices.filter(function (dice) { return dice.uuid != selectedDice.uuid});
        this.scene.remove(selectedDice);
        if (selectedDice.body) this.world.remove(selectedDice.body);
        this.renderer.render(this.scene, this.camera);

        return get_dice_value(selectedDice);
    }
    ...
    this.store_box = function(container) {
        this.use_adapvite_timestep = true;
        this.animate_selector = true;

        this.store_scene = new THREE.Scene();
        this.world = new CANNON.World();
        ...
        var ambientLight = new THREE.AmbientLight(that.ambient_light_color);
        this.store_scene.add(ambientLight);
        ...
        this.renderer.render(this.store_scene, this.camera);
    }

    this.store_box.prototype.reinit = function(container) {
        this.cw = container.clientWidth / 2;
        this.ch = container.clientHeight / 2;
        this.w = this.cw;
        this.h = this.ch;
        ...
        if (this.camera) this.store_scene.remove(this.camera);
        this.camera = new THREE.PerspectiveCamera(20, this.cw / this.ch, 1, this.wh * 1.3);
        this.camera.position.z = this.wh;

        var mw = Math.max(this.w, this.h);
        if (this.light) this.store_scene.remove(this.light);
        this.light = new THREE.SpotLight(that.spot_light_color, 2.0);
        this.light.position.set(-mw / 2, mw / 2, mw * 2);
        this.light.target.position.set(0, 0, 0);
        this.light.distance = mw * 5;
        ...
        this.light.shadowMapWidth = 1024;
        this.light.shadowMapHeight = 1024;
        this.store_scene.add(this.light);

        if (this.desk) this.store_scene.remove(this.desk);
        this.desk = new THREE.Mesh(new THREE.PlaneGeometry(this.w * 2, this.h * 2, 1, 1), new THREE.MeshPhongMaterial({ color: that.desk_color }));
        this.desk.receiveShadow = that.use_shadows;
        this.store_scene.add(this.desk);

        this.renderer.render(this.store_scene, this.camera);
    }

    this.store_box.prototype.store_dice = function(storeDice) {
        while (this.store_scene.children.length > 3) {
            this.store_scene.remove(this.store_scene.children[3]);
        }

        var step = this.w / 18;
        for (var i = 0, pos = -6; i < storeDice.length; ++i, pos += 3) {
            var dice = $t.dice['create_d6']();
            dice.position.set(pos * step, 0, 20);
            dice.rotation.z += 3.15;
            dice.castShadow = true;
            dice.dice_type = 'd6';

            shift_dice_faces(dice, storeDice[i], 6);

            this.store_scene.add(dice);
        }
